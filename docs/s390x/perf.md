...
Last updated: 2026-04-09 20:38 PDT
...
| --- | --- | --- | --- |
| [tests/s390x/perf/iterator_table.lua](/Users/kaitlyndavis/dev/github.com/k8ika0s/luajit2-s390x/tests/s390x/perf/iterator_table.lua) | `iterator_table` | `pairs_sum`, `pairs_array_sum` | retained exact root-ITERN and root-ITERL blacklist wins; residual iterator attribution remains active |
...
| [tests/s390x/perf/ffi_cdata.lua](/Users/kaitlyndavis/dev/github.com/k8ika0s/luajit2-s390x/tests/s390x/perf/ffi_cdata.lua) | `ffi_cdata` | `pair_loop`, `mixed_width_loop` | retained pair-loop save-time win; mixed-width regression screen |
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `pairs_sum/hot` | `iterator_table` | `0.011391` | `0.004135` | `+0.007256`, `2.75x` | `kdz` | `2026-04-09 20:38 PDT` | retained exact root-ITERN plus root-ITERL blacklist wins; host-pair clean |
| `pairs_array_sum/hot` | `iterator_table` | `0.008139` | `0.003651` | `+0.004488`, `2.23x` | `kdz` | `2026-04-09 20:38 PDT` | retained exact root-ITERL blacklist win; former dominant array-side row collapsed |
...
| `pair_loop/hot` | `ffi_cdata` | `0.023094` | `0.017319` | `+0.005775`, `1.33x` | `kdz` | `2026-04-09 15:59 PDT` | retained exact pair-loop save-time `BC_TGETB` DONE win; now a regression screen |
| `mixed_width_loop/hot` | `ffi_cdata` | `0.027481` | `0.027969` | `-0.000488`, `0.98x` | `kdz` | `2026-04-09 15:59 PDT` | sibling under the retained pair-loop save-time win; near parity |
...
Current frontier after the retained `iterator_table` root-ITERN and root-ITERL blacklist wins:
...
- the active engineering frontier stays on `iterator_table` for one fresh
  post-ITERL attribution before reranking to `mixed_ffi`
- retained iterator cut:
  ...
  - exact mechanism:
    ...
    - after those two roots are blacklisted, blacklist the newly exposed exact
      root `BC_ITERL` array loop trace:
      `nsnap=2 nins=32798 mcloop=512`
    - this removes the `root=3` `BC_JMP` exit-0 loop-descendant chain on the
      official full row
  - host-pair result:
    - `kdz`: candidate rerun `pairs_sum/hot 0.011391`,
      `pairs_array_sum/hot 0.008139`; immediate disabled-env control
      `0.011327` and `0.072042`
    - `zkd0`: candidate rerun `pairs_sum/hot 0.012644`,
      `pairs_array_sum/hot 0.011131`; immediate disabled-env control
      `0.020846` and `0.125596`
  - read:
    - the root-ITERL blacklist collapses the former dominant
      `pairs_array_sum/hot` row
    - both iterator hot rows are now in the same residual band, so the next
      iterator step is fresh post-blacklist attribution rather than another
      root-loop blacklist by default
...
- `vararg_paths` is no longer treated as a parked dominated family on the
  ...
  - read:
    ...
    - `sum_loop` remains a carried red row, but it is not the active lane
...
- the retained mixed floor now includes the first host-pair-clean
  `lj_vm_next` lowering win:
  - in [src/lj_asm_s390x.h](/Users/kaitlyndavis/dev/github.com/k8ika0s/luajit2-s390x/src/lj_asm_s390x.h)
    `asm_gencall_sload()`, hidden `IRSLOAD_KEYINDEX` call arguments now reuse
    live `RID_BASE` directly instead of rematerializing `jit_base` into a
    scratch GPR before loading the helper argument
  - retained host-pair result:
    - `kdz`: `mixed_loop/hot 0.012123`, `-joff 0.003734`
    - `zkd0`: `mixed_loop/hot 0.014944`, `-joff 0.004387`
- retained-floor mixed attribution on `kdz` still points at the `pairs(map)`
  family as the dominant residual payer:
  - isolated split timings on the retained env bundle:
    - JIT-on:
      - `band_only 0.001170`
    ...
    - `-joff`:
      ...
      - `full 0.218688`
  - read:
    - under the retained floor, the residual JIT cost is still dominated by
      the `pairs(map)` subpath, not by the outer numeric/select/ipairs body or
      by a general caller-continuation tax
    - the recorder-side `sidecheck_interp` / nil-descendant tranche is now
      exhausted as a profitable local edit surface
    - the fresh official-row attribution still named the root-owned
      `parent=2 exit=1` runtime family as dominant
    - the first broader `lj_vm_next` runtime-handoff attempt after that
      attribution was exact and mechanism-real, but catastrophically slower on
      `kdz`, so the current `mixed_noffi` runtime lane is explicitly closed
- the retained mixed bundle now includes:
...
- regression screens on the retained candidate stayed clean on `kdz`:
  - `dispatch_trace`
    - `numeric_loop/hot 0.000156`
    - `side_exit_loop/hot 0.000372`
    - `hotexit_loop/hot 0.001033`
  - `iterator_table`
    - `pairs_sum/hot 0.066581`
    - `pairs_array_sum/hot 0.068733`
- read:
  - the root-2 bridge cut remains part of the retained mixed floor, but the
    current retained edge is now one level later in the `lj_vm_next` call-arg
    lowering path
