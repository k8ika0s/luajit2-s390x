...
#define MCODE_JUMPRANGE	(((uintptr_t)1) << LJ_TARGET_JUMPRANGE)
#define MCODE_RANGE64	(MCODE_JUMPRANGE - (uintptr_t)0x10000u)
...
static void mcode_setrange(jit_State *J, uintptr_t addr)
{
  ...
  J->mcmin = addr & ~(MCODE_JUMPRANGE - 1);
  J->mcmax = J->mcmin + MCODE_JUMPRANGE;
#else
