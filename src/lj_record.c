...
static LoopEvent rec_itern(jit_State *J, BCReg ra, BCReg rb)
{
  ...
  /* s390x can observe a live hash payload value before the helper tuple has
  ** re-materialized the boxed key. Preserve the value slot and treat the
  ** iterator as still carrying loop payload so the trace mirrors VM state.
  */
...
#define rcv	(&ix.keyv)
  ...
  case BC_JFORL: {
    ...
    rec_loop_jit(J, rc, pc, rec_for(J, fori, 1));
    break;
    }
  case BC_JITERL:
    rec_loop_jit(J, rc, NULL, rec_iterl(J, traceref(J, rc)->startins));
    break;
  case BC_JLOOP:
    rec_loop_jit(J, rc, NULL, rec_loop(J, ra,
...
void lj_record_setup(jit_State *J)
{
  ...
  if (J->parent) {  /* Side trace. */
	...
	fprintf(stderr,
      ...
      /* Side traces that enter an already compiled FOR loop need the iterator
      ** state in the side trace before the side-exit limit checks run.
      */
