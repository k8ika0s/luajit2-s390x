				       ...
				       int cc)
{
  ...
  if (!as->loopref || ref >= as->loopref || op != IR_UGT ||
      ...
      !irref_isk(rref) ||
      !asm_s390x_open_upvalue_delta_ref(as, lref))
    return 0;

  /* Only fold guards that all exit through the initial snapshot. Strengthening
  ** the first guard is safe here because the duplicate range checks precede
  ** any later snapshot and no non-guard side effects are allowed in between.
  */
  ...
  if (first == 0)
    return 0;
  if (ref != first)
    return 1;  /* The first guard is strengthened to cover this one. */
...
static int asm_s390x_fill_loop_crj_nop(ASMState *as, uint16_t ins)
{
  uint8_t *p;
  if (as->loopinv != 3)
    return 0;
  p = (uint8_t *)(as->mctop - 3);
...
static int asm_s390x_loop_crj_nop_available(ASMState *as)
{
  uint8_t *p;
  if (as->loopinv != 3)
    return 0;
  p = (uint8_t *)(as->mctop - 3);
  return p[0] == 0x07 && p[1] == 0x07;
}
						      ...
						      IRRef *srcrefp)
{
  ...
  if (irref_isk(srcref) || bswap->op1 != srcref ||
      ...
      !asm_s390x_match_bsar_seed_xor(as, seedref, srcref, &sh) ||
      sh != 2)
    return 0;
...
static int asm_add_bxor_bswap_bnot_loop_tail(ASMState *as, IRIns *ir, Reg dest)
{
  IRRef baseref = 0;
  IRRef bnotref = 0, innerref = 0, bswapref = 0, accref = 0, srcref;
  IRIns *tail, *bnot, *inner, *bswap, *accir;
  Reg base, src, acc, tmp;
  RegSet allow;
  ...
  src = ra_alloc1_nobase(as, srcref, rset_exclude(RSET_GPR_NOB, base), -232);
  allow = rset_exclude(rset_exclude(RSET_GPR_NOB, base), src);
  acc = ra_alloc1_nobase(as, accref, allow, -233);
  tmp = ra_scratch(as, rset_exclude(allow, acc));
  asm_s390x_bitop_log(as, "add_bxor_bswap_bnot_loop_tail", tail, tmp, src, acc, 0);
  if (base == dest) {
    emit_u16_pair(as, S390X_INS_RR(S390XI_XR, tmp, acc),
		  S390X_INS_RR(S390XI_AR, base, tmp));
  } else {
    emit_u32(as, S390X_INS_RRF_M(S390XI_ARK, dest, tmp, base));
    emit_u16_pad4(as, S390X_INS_RR(S390XI_XR, tmp, acc));
  }
  emit_u16_u48(as, S390X_INS_RR(S390XI_XR, tmp, src),
	       S390X_INS_RIL(S390XI_XILF, tmp, 0xffffffffu));
  emit_u32(as, S390X_INS_RXE(S390XI_LRVR, tmp, src));
  return 1;
}
...
static void asm_add(ASMState *as, IRIns *ir)
{
  ...
  if (asm_add_bxor_mix_pos_loop_tail(as, ir, dest))
    return;
  if (asm_add_bxor_bswap_bnot_loop_tail(as, ir, dest))
    return;
  if (!irt_isguard(ir->t) && irt_isinteger(ir->t) && !irref_isk(ir->op2)) {
    IRIns *lir = IR(ir->op1);
    IRIns *rir = IR(ir->op2);
  ...
  if (irref_isk(ir->op2)) {
	...
	int low32addk = bnorm &&
	  (suffix200add || asm_s390x_addk1_bitop_loop_carry(as, ir) ||
	...
	if (bnorm && !low32addk)
	  asm_bnorm32(as, ir, dest);
	if (low32addk && k == 1 && !irt_isguard(ir->t) && dest == left &&
	    (suffix200add || asm_s390x_addk1_bitop_loop_carry(as, ir)) &&
	    asm_s390x_loop_crj_nop_available(as)) {
	  Reg addk = ra_allock(as, emitk, rset_exclude(RSET_GPR_NOB, left));
	  uint16_t ar = S390X_INS_RR(S390XI_AR, dest, addk);
	  if ((suffix200add &&
	       asm_s390x_fill_loop_crj_never_taken(as, ar)) ||
	      asm_s390x_fill_loop_crj_nop(as, ar))
	    return;
	}
	if (!irt_isguard(ir->t) && dest != left)
	  emit_u48_pad8(as, S390X_INS_RIE_D(low32addk ? S390XI_AHIK :
					    S390XI_AGHIK, dest, left, emitk));
	else
...
static void asm_equal(ASMState *as, IRIns *ir)
{
  ...
  cmp_right = right;
  if (cmp32s) {
    RegSet allow = rset_exclude(rset_exclude(RSET_GPR_NOB, left), right);
    cmp_left = ra_scratch(as, allow);
    cmp_right = ra_scratch(as, rset_exclude(allow, cmp_left));
  }
  asm_guardcc(as, ir->o == IR_EQ ? CC_NE : CC_EQ);
...
dotypecheck:
  ...
  if (ir->op2 & IRSLOAD_TYPECHECK) {
	...
	fprintf(stderr,
      ...
      asm_guardcc(as, CC_NE);
      if (ir->op1 == 4 &&
	  (ir->op2 & IRSLOAD_MODEMASK) == (IRSLOAD_INHERIT|IRSLOAD_TYPECHECK) &&
	  asm_s390x_forl_current_compare_fix_enabled()) {
	if (LJ_GC64 && asm_s390x_gc64_signed_int_sload_enabled()) {
	  emit_u32(as, S390X_INS_RI(S390XI_CGHI, tmp,
				    (int32_t)LJ_TISNUM));
	  emit_shiftimm(as, S390XI_SRAG, tmp, tmp, 47);
	} else {
	  expected = ra_scratch(as, rset_exclude(tallow, tmp));
