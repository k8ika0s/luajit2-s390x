...
for _, scale in ipairs(bench.scale_order(scales)) do
  ...
  local sub_large_expected = sub_large(n)
  local cmp_large_expected = cmp_large(n)
  ...
  cases[#cases + 1] = {
    ...
    run = sub_large,
    validate = function(result)
      bench.eq(result, sub_large_expected, "sub_large/" .. scale)
    end,
  }
  cases[#cases + 1] = {
    workload = "cmp_large",
    ...
    run = cmp_large,
    validate = function(result)
      bench.eq(result, cmp_large_expected, "cmp_large/" .. scale)
    end,
  }
  cases[#cases + 1] = {
    workload = "aref_small",
